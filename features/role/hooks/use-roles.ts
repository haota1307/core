 ...
 */
export function useRoles(query: GetRolesQuery, options?: { enabled?: boolean }) {
  return useQuery({
...
export function useUpdateRolePermissions(roleId: string) {
  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      ...
      toast.success(data.message || t("permissionsUpdateSuccess"));
    },
    onError: (error: any) => {
      toast.error(error.message || t("permissionsUpdateError"));
    },
  });
}
