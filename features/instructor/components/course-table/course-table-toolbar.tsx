...
interface CourseTableToolbarProps<TData> {
  table: Table<TData>;
  searchValue: string;
...
export function CourseTableToolbar<TData>({
  table,
  searchValue,
  onSearchChange,
  statusFilter,
  ...
  levelFilter,
  onLevelFilterChange,
}: CourseTableToolbarProps<TData>) {
  const t = useTranslations("instructor.courseTable");
  const isFiltered =
